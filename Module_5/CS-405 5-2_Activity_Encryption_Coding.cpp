// CS-405 5-2_Activity_Encryption_Coding.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
{
    std::string file_text = "Eric E. Farkas\nThis is my test string\n";
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    try
    {
        ...
        if (writeFile.is_open())
        {
            ...
            // Write the timestamp as of the time of program running
            char buffer[80];
            struct tm ltm;
            time_t now = time(0);
            localtime_s(&ltm, &now);
