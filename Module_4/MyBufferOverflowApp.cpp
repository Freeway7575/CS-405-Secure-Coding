...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        if (start + (increment * steps) > std::numeric_limits<T>::max()) // Checks over each increment for overflow/underflow. If found, return -1
        {
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (start - (decrement * steps) < std::numeric_limits<T>::min()) // Checks over each decrement for underflow/overflow. If found, return -1
        {
